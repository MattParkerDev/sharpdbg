...
public partial class ManagedDebugger : IDisposable
{
    ...
    public List<VariableInfo> GetVariables(int variablesReference)
    {
        ...
        try
        {
	        ...
	        if (localsScope.Frame.Arguments.Length is 0) return result;
	        ...
	        {
		        var implicitThisValue = localsScope.Frame.Arguments[0];
		        var variableInfo = new VariableInfo
		        {
	        ...
	        foreach (var (index, argumentCorDebugValue) in localsScope.Frame.Arguments.Skip(skipCount).Index())
	        {
